...
void call_pwd(char *home) // carries out pwd
{
  char string[1000] = "";
...
void call_exec(char **list, int *background_ptr, node **head_ptr) // executes shell commands
{
  int pid = fork();
...
void call(char *query, char *home, int *background_ptr, node **head_ptr) // calls the other call_* functions
{
  if(!strcmp(query, "\n") || !strcmp(query, ""))
    return;
  ...
  {
    call_exec(list, background_ptr, head_ptr);
  }
